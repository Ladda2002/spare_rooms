...
if(isset($_POST["submit"])){
  if($_POST["id"] == ""){
    saveUser($_POST["username"],$_POST["password"],$_POST["address"],$_POST["email"],$_POST["phone"],$_POST["gender"],$_POST["status"],$_POST["role"],$_FILES["profile_img"]["name"]);
  }else{
    editUser($_POST["id"],$_POST["username"],$_POST["password"],$_POST["address"],$_POST["email"],$_POST["phone"],$_POST["gender"],$_POST["status"],$_POST["role"],$_FILES["profile_img"]["name"]);
  }
}

if($_GET["id"] == ""){
  $txtHead = "เพิ่ม ข้อมูลผู้ใช้งาน";
}else{
  $txtHead = "แก้ไข ข้อมูลผู้ใช้งาน";
}
?>
<body>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <select name="role" class="form-control" required>
                        ...
                        <option value="1" <?php if($currentUser['role'] == 1){ ?> selected<?php } ?>>ผู้ดูแลระบบ</option>
                        <option value="2" <?php if($currentUser['role'] == 2){ ?> selected<?php } ?>>เจ้าของหอ</option>
                        <option value="3" <?php if($currentUser['role'] == 3){ ?> selected<?php } ?>>ผู้ขายประกัน</option>
                        <option value="4" <?php if($currentUser['role'] == 4){ ?> selected<?php } ?>>เจ้าของห้อง</option>
                        <option value="5" <?php if($currentUser['role'] == 5){ ?> selected<?php } ?>>ผู้หาห้องเช่า</option>
                      </select>
                    </div>
                  </div>
                  <div class="col-md-6">
